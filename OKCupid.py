# -*- coding: utf-8 -*-
"""
Created on Fri Jan 22 19:53:31 2021
...
filename = "C:\\Users\\Rafa\\Desktop\\Master Big Data\\Modulo 5\\OKCupid\\profiles.csv"
...
print("{} males ({:.1%}), {} females ({:.1%})".format(len(male),len(male)/len(d), len(female),len(female)/len(d)))

# Ignore columns with "essay" in the name (they are long)
 # Prettyprints pandas dataframes
  # Sample the first 10 rows
PrettyPandas(d.head(10)[[c for c in d.columns if "essay" not in c]]) # Ignore columns with "essay" in the name (they are long)
...
sns.distplot(male["age"], ax=ax1, bins=range(d["age"].min(),d["age"].max()), kde=False, color="g")
ax1.set_title("Age distribution for males")

sns.distplot(female["age"], ax=ax2, bins=range(d["age"].min(),d["age"].max()), kde=False, color="b")
...
sns.distplot(male["age"], ax=ax1, bins=range(d["age"].min(),d["age"].max()), kde=False, color="g", label="males")
sns.distplot(female["age"], ax=ax1, bins=range(d["age"].min(),d["age"].max()), kde=False, color="b", label="females")
ax1.set_ylabel("Number of users in age group")
ax1.set_xlabel("")
ax1.legend()
